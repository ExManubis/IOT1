...
try: #Finder info inde i credentials
...
def do_connect(): #Begynder at forbinde til wifi
    if not wifi.isconnected(): 
        ...
        MAX_ATTEMPTS = 20
        attempt_count = 0
        while not wifi.isconnected() and attempt_count < MAX_ATTEMPTS: #hvis den ikke kan få wifi pluser den med 1 
            attempt_count += 1
            
            utime.sleep(1)

        if attempt_count == MAX_ATTEMPTS: #Hvis den har prøvet 20 gange stopper den
